...
pub async fn handle_admin_message(ctx: Context, msg: Message) -> Result<(), ()> {
    if msg.content.starts_with("!update") {
        ...
        if msg.content.len() == 6 {
            msg.reply(&ctx.http, "The following argumenst are needed: {admin|image_provider} {userid}").await.unwrap();
        }
        else {
            let rooli = &msg.content[msg.content.find(' ').unwrap()+1..msg.content.rfind(' ').unwrap()];
            let userid = &msg.content[msg.content.rfind(' ').unwrap()+1..msg.content.len()].parse::<u64>().unwrap();
            match rooli {
                "admin" => {
                ...
                _ => {
                    msg.reply(&ctx.http, "Invalid role provided!").await.unwrap();
                    return Ok(());
                }
            }
