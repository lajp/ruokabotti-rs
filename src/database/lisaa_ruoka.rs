...
impl Database {
    ...
    pub async fn lisaa_uusi_ruoka(&self, nimi: String) -> Result<i32, sqlx::Error> {
        ...
        match res.rows_affected() {
            r if r > 0 => {
                Ok(self.nouda_ruoka_by_name(nimi).await.unwrap().RuokaID)
            }
            _ => {
    ...
    pub async fn lisaa_ruoat_listaan(&self, lista: Vec<(NaiveDate, String)>) -> Result<(), ()> {
        ...
        for item in lista {
            ...
            let foodid = match self.nouda_ruoka_by_name(ruokaname.to_string()).await {
                Ok(r) => r.RuokaID,
                Err(_) => {
                    self.lisaa_uusi_ruoka(ruokaname.to_string()).await.unwrap()
                }
            };
            let res = sqlx::query!("INSERT INTO Ruokalista (PVM, RuokaID, KokoRuoka) VALUES( ? , ? , ?)", &item.0.to_string(), &foodid, &item.1)
                .execute(&mut conn).await;
            match res {
                Ok(_) => {
                    continue
