...
impl Database {
    pub async fn lisaa_arvio(&self, userid: u64, reaktio: i32, ruoka: String) {
        ...
        sqlx::query!("INSERT INTO Arvostelut (RuokaID, KayttajaID, Arvosana) VALUES( ? , ? , ?)",
            ruokaid, userid, reaktio).execute(&mut conn).await.unwrap();
    }
    pub async fn poista_arvio(&self, userid: u64, reaktio: i32, ruoka: String) {
        ...
        let mut conn = self.pool.acquire().await.unwrap();
        sqlx::query!("DELETE FROM Arvostelut WHERE KayttajaID = ? AND Arvosana = ? AND RuokaID = ?",
            userid, reaktio, ruokaid).execute(&mut conn).await.unwrap();
    }
    ...
    pub async fn anna_kayttajan_statistiikka(&self, userid: u64) -> Option<KayttajaStatistiikka> {
        let mut conn = self.pool.acquire().await.unwrap();
        let maara = sqlx::query!("SELECT COUNT(DISTINCT(RuokaID)) AS maara FROM Arvostelut WHERE KayttajaID = ?", userid).fetch_one(&mut conn).await.unwrap().maara;
        match maara {
            0 => return None,
            _ => (),
        }
